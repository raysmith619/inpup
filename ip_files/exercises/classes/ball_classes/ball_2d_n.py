#ball_2d_n.py 11Oct2021  crs, from ball_2d.py
#             12Aug2021  crs, Author
"""
Two dimentional ball with number
"""
import time

class Ball2dN:
    next_id = 1          # Next unique id
    next_number = 1      # Next ball number
    ...
    def __init__(self, id=None, number=None,
        ...
        :id: unique identification
            default: generated
        :number: ball's number
                default: generated, starting at 1
                        increasing, 2,3,...
        ...
        self.id = id
        if number is None:
           number = Ball2dN.next_number
           Ball2dN.next_number += 1
    ...
    def __str__(self):
        ...
        """
        ball_str = f"Ball2dN[{self.id}]: {self.number} {self.color}"
        ball_str += f" r:{self.radius}"
        ball_str += f" x:{self.pos_x:.5} y:{self.pos_y:.5}"
        ball_str += f" vx:{self.vel_x:.5} vy:{self.vel_y:.5}"
    ...
    def new_time(self, now=None):
        """ Adjust per new time
        :now: New time in seconds since previous time
                default: set as current time
        """
        if now is None:
            now = time.time()
        time_delta = now - self.time_prev
        if time_delta != 0:
            self.pos_x += (self.vel_x*time_delta)
            self.pos_y += (self.vel_y*time_delta)
        self.time_prev = now    # Record time
        
# Self Test
if __name__ == "__main__":
    b1 = Ball2dN()
    b2 = Ball2dN(color="blue", fill="blue", radius=3,
                pos_x=2., pos_y=4.)
    b3 = Ball2dN(color="red", radius=2, vel_x=2., vel_y=3.)
    for i in range(4):
        print(f"{i:4}\n\tb1: {b1}\n\tb2: {b2}\n\tb3: {b3}")
        b1.new_time()
        b2.new_time()
