# chess_example.py  02Feb2022  crs, from stackoverflow

import tkinter as tk        # Changed from Tkinter
...
if __name__ == "__main__":
    root = tk.Tk()
    size = 100
    board = GameBoard(root,size=size)
    board.pack(side="top", fill="both", expand="true", padx=4, pady=4)
    player1 = tk.PhotoImage(data=imagedata)
    player1 = player1.zoom(5,5)
    board.addpiece("player1", player1, 0,0)
    board.addpiece("player3", player1, 0,7)
    board.addpiece("player4", player1, 7,0)
    board.addpiece("player2", player1, 7,7)
    root.mainloop()
